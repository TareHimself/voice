...
from constants import VOSK_MODEL_URL, TTS_DIR
    ...
    def run(self):
        if SHOULD_USE_INPUT:
            ...
            if not model_downloaded:
                if not path.exists(path.join(getcwd(), TTS_DIR)):
                    if model_downloading:
                        ...
                        z.close()
                        result.close()
                        os.rename(path.join(getcwd(), dir_name), path.join(getcwd(), TTS_DIR))
                        model_downloaded = True
                        model_downloading = False
                else:
                    model_downloaded = True

            self.model = Model(model_path=path.join(getcwd(), TTS_DIR))
